...
def image_binary_content_zero_lowest_bit(img_path):
    ...
    To show the returned image makes conversion to uint8
...
def embed_text_in_image(binary_text, input_array):
    """
    Gets a text, takes its binary value, calculates its length and put its length in the first two bytes of the image.
    :param binary_text: binary text to be embedded at the least significant bits (lsb) of every pixel.
...
def embed_encoded_msg_in_image(img_path, msg_to_encode):
    """
    Takes the path to an image, and encode the input message inside the lsb of the image
...
img_path = os.path.join(os.getcwd(), '2.jpeg')
msg_to_encode = 'Hello, steganography is awesome!'
